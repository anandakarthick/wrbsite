<?php
// Generate tennextsoft.com website: 5-page static site, emerald/teal theme
$base = dirname(__DIR__);
$dir = $base . '/public/sites/tennextsoft';
@mkdir($dir, 0777, true);

// copy images into the site folder (self-contained docroot)
$images = [
    'hero.jpg' => 'images/tech/analyst-screens.jpg',
    'about.jpg' => 'images/tech/engineer-laptop.jpg',
    'svc-web.jpg' => 'images/cards/insightiq.jpg',
    'svc-mobile.jpg' => 'images/cards/convodesk.jpg',
    'svc-ecom.jpg' => 'images/cards/kartpos.jpg',
    'office.jpg' => 'images/office/team-working.jpg',
    'work1.jpg' => 'images/cards/documind.jpg',
    'work2.jpg' => 'images/tech/circuit-board.jpg',
    'work3.jpg' => 'images/cards/peoplecore.jpg',
];
...
$NAME = 'Tennext Soft';
$SHORT = 'Tennext Soft';
$PHONE = '+91 98404 22538';
$PHONE_RAW = '919840422538';
$EMAIL = 'contact@tennextsoft.com';
$ADDRESS = 'Tambaram, Chennai - 600045, Tamil Nadu, India';

$CSS = <<<CSS
:root{--g1:#4f46e5;--g2:#0284c7;--g3:#f59e0b;--ink:#151833;--body:#333a56;--muted:#5a6280;--soft:#767e9b;--bg:#f7f8fd;--grad:linear-gradient(135deg,#4f46e5 0%,#0284c7 55%,#f59e0b 100%)}
...
.btn-primary{background:var(--grad);color:#fff;box-shadow:0 8px 20px rgba(79,70,229,.3)}
.btn-primary:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(2,132,199,.4)}
.btn-outline{background:#fff;color:var(--g1);border:2px solid rgba(79,70,229,.35)}
.btn-outline:hover{background:rgba(79,70,229,.07)}
...
.totop{position:fixed;bottom:1.6rem;left:1.6rem;width:46px;height:46px;border-radius:50%;background:var(--grad);color:#fff;border:none;cursor:pointer;font-size:1rem;box-shadow:0 10px 24px rgba(79,70,229,.35);opacity:0;visibility:hidden;transform:translateY(10px);transition:.3s;z-index:150}
...
header{position:fixed;top:0;left:0;right:0;z-index:100;background:rgba(255,255,255,.85);backdrop-filter:blur(14px);border-bottom:1px solid rgba(21,24,51,.06);transition:.3s}
header.scrolled{background:rgba(255,255,255,.97);box-shadow:0 10px 24px rgba(21,24,51,.08)}
...
.badge{display:inline-block;padding:.45rem 1.1rem;background:rgba(79,70,229,.09);border:1px solid rgba(79,70,229,.3);border-radius:.5rem;font-size:.8rem;font-weight:700;color:var(--g1);margin-bottom:1rem;letter-spacing:.04em;text-transform:uppercase}
...
.hero{padding:9.5rem 0 0;background:linear-gradient(160deg,#eef1fb 0%,#f7f8fd 55%,#fdf6e9 100%);position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;width:520px;height:520px;border-radius:50%;background:radial-gradient(circle,rgba(245,158,11,.16),transparent 65%);top:-180px;right:-140px}
.hero::after{content:'';position:absolute;width:420px;height:420px;border-radius:50%;background:radial-gradient(circle,rgba(2,132,199,.14),transparent 65%);bottom:-80px;left:-120px}
...
.hero-img img{display:block;width:100%;border-radius:1.4rem;box-shadow:0 30px 60px rgba(21,24,51,.2);border:6px solid #fff}
...
.page-hero{padding:8.5rem 0 3.5rem;background:linear-gradient(135deg,#312e81,#1e40af);color:#fff;position:relative;overflow:hidden}
.page-hero::after{content:'';position:absolute;width:380px;height:380px;border-radius:50%;background:rgba(245,158,11,.2);filter:blur(70px);top:-120px;right:-80px}
...
.card{background:#fff;border:1px solid rgba(21,24,51,.07);border-left:4px solid var(--g1);border-radius:1rem;padding:1.7rem;transition:.35s}
.card:hover{transform:translateY(-6px);box-shadow:0 18px 40px rgba(79,70,229,.16);border-left-color:var(--g3)}
...
.img-card{background:#fff;border:1px solid rgba(21,24,51,.07);border-radius:1rem;overflow:hidden;transition:.35s}
.img-card:hover{transform:translateY(-6px);box-shadow:0 18px 40px rgba(79,70,229,.16)}
...
.checklist li{display:flex;gap:.7rem;background:#fff;border:1px solid rgba(21,24,51,.07);border-radius:.8rem;padding:.95rem 1.15rem;font-size:.93rem;transition:.3s}
.checklist li:hover{transform:translateY(-3px);border-color:rgba(79,70,229,.4)}
...
.step::before{content:'';position:absolute;left:21px;top:46px;bottom:0;width:2px;background:rgba(79,70,229,.25)}
...
.plan{background:#fff;border:1px solid rgba(21,24,51,.08);border-radius:1rem;padding:1.9rem 1.6rem;display:flex;flex-direction:column;transition:.35s;position:relative}
.plan:hover{transform:translateY(-7px);box-shadow:0 18px 40px rgba(79,70,229,.16)}
...
.quote{margin-top:2.4rem;padding:2rem;background:linear-gradient(135deg,rgba(79,70,229,.06),rgba(245,158,11,.07));border:1px solid rgba(79,70,229,.2);border-radius:1rem}
.quote blockquote{font-style:italic;color:#20264a;font-size:1.02rem;margin-bottom:1rem}
.quote b{color:var(--ink);font-size:.9rem}.quote span{color:var(--soft);font-size:.8rem;display:block}
details{background:#fff;border:1px solid rgba(21,24,51,.07);border-radius:.8rem;margin-bottom:.7rem;overflow:hidden}
details[open]{border-color:rgba(79,70,229,.4)}
summary{padding:1rem 1.35rem;cursor:pointer;font-weight:600;color:#12332а;color:#1b2040;list-style:none;display:flex;justify-content:space-between;gap:1rem}
...
.ccard{background:#fff;border:1px solid rgba(21,24,51,.07);border-radius:1rem;padding:1.6rem;text-align:center;transition:.3s}
.ccard:hover{transform:translateY(-5px);border-color:rgba(79,70,229,.4)}
...
.split img{border-radius:1.2rem;box-shadow:0 22px 45px rgba(21,24,51,.15)}
footer{background:#111327;color:#a5accd;padding:3rem 0 1.5rem}
...
footer a{color:#a5accd;font-size:.9rem}
footer a:hover{color:#f59e0b}
.fbrand{display:flex;align-items:center;gap:.6rem;font-weight:800;font-size:1.05rem;color:#fff;margin-bottom:.8rem}
.fbottom{border-top:1px solid rgba(255,255,255,.12);padding-top:1.3rem;text-align:center;font-size:.83rem;color:#7d86a8}
.fbottom a{color:#f59e0b;font-weight:600}
...
@media(max-width:900px){
 ...
 .nav ul{position:fixed;top:64px;right:-100%;width:78%;max-width:300px;height:100vh;background:rgba(255,255,255,.98);flex-direction:column;align-items:flex-start;padding:2rem;gap:1.2rem;transition:.3s;box-shadow:-10px 0 30px rgba(21,24,51,.12)}
 .nav ul.open{right:0}
...
/* ===== boxy indigo/amber style overrides ===== */
.btn{border-radius:.35rem}
.card,.img-card,.checklist li,.plan,.quote,details,.ccard{border-radius:.4rem}
.badge{border-radius:.25rem;background:#151833;color:#fbbf24;border:none}
.brand-mark{border-radius:.35rem;transform:none}
.hero-img img{border-radius:.5rem;border-width:0;outline:6px solid #fff;outline-offset:-6px}
.hero-img::after{border-radius:.3rem;inset:auto -14px -14px auto;background:#f59e0b}
.hero-grid > div:first-child{order:2}
.hero-grid > .hero-img{order:1}
.nav ul a{text-transform:uppercase;font-size:.78rem;letter-spacing:.08em;font-weight:600}
.sec-title{position:relative;padding-bottom:.7rem}
.sec-title::after{content:"";position:absolute;left:0;bottom:0;width:56px;height:4px;background:#f59e0b}
.center .sec-title::after{left:50%;transform:translateX(-50%)}
.card{border:1.5px solid rgba(21,24,51,.14);border-left:1.5px solid rgba(21,24,51,.14);box-shadow:none}
.card:hover{border-color:#f59e0b;box-shadow:0 14px 30px rgba(79,70,229,.14)}
.icon{border-radius:.35rem}
.step-n{border-radius:.35rem}
.cta-band{border-radius:.5rem}
.page-hero p{border-left:3px solid #f59e0b;padding-left:.9rem}
CSS;
...
<header>
  <nav class="nav">
    <a href="index.html" class="brand"><img src="logo.png" alt="Tennext Soft logo" class="brand-logo"><span>Tennext Soft<small>Software Company</small></span></a>
    <ul>{$links}</ul>
...
<footer>
  <div class="container">
    <div class="fgrid">
      <div>
        <div class="fbrand"><img src="logo.png" alt="Tennext Soft logo" class="brand-logo"> Tennext Soft</div>
      ...
      <div>
        <h4>Contact</h4>
        <ul>
          <li><a href="tel:+919840422538">📞 {$PHONE}</a></li>
          <li><a href="mailto:{$EMAIL}">✉ {$EMAIL}</a></li>
          <li>📍 {$ADDRESS}</li>
        </ul>
      </div>
    </div>
    <div class="fbottom">© <span class="yr"></span> Tennext Soft · All rights reserved · <a href="https://tennextsoft.com/">tennextsoft.com</a></div>
  </div>
</footer>
<a class="wa-float" href="https://wa.me/919840422538?text=Hi%20Tennext%20Soft!%20I%20need%20a%20software%20solution." target="_blank" rel="noopener" aria-label="WhatsApp">
...
<section class="hero">
  <div class="container hero-grid">
    ...
    <div class="hero-img reveal"><img src="hero.jpg" alt="Technology solutions by Tennext Soft"></div>
  </div>
</section>
...
<section style="background:linear-gradient(180deg,#eef1fb,#f7f8fd)">
  <div class="container split">
    <img src="about.jpg" alt="Our team at work" class="reveal">
    <div>
      <span class="badge">Why Tennext Soft</span>
...
<section>
  <div class="container">
    <div class="quote reveal">
      ...
      <b>Prakash M</b><span>Textile Shop Owner, Tambaram</span>
    </div>
...
file_put_contents("$dir/index.html", shell(
    'Tennext Soft — Websites, Apps & Business Software in Chennai',
    'Website development, mobile apps, e-commerce, billing and custom software in Tambaram, Chennai. Call +91 98404 22538.',
...
<section style="background:linear-gradient(180deg,#eef1fb,#f7f8fd)">
  <div class="container">
    ...
    <div class="steps">
      <div class="step reveal"><div class="step-n">1</div><div><h3>Free Discussion</h3><p>Tell us your need — over call, WhatsApp or chai at our Tambaram office. We understand before we quote.</p></div></div>
...
<section>
  <div class="container split">
    <div>
      ...
      <p style="color:var(--muted);margin-bottom:1rem">Tennext Soft is a software company based in Tambaram, Chennai. We started with a simple observation: small and medium businesses around us were paying too much for software they didn't understand, or struggling with tools that didn't fit how they actually work.</p>
    ...
    <img src="office.jpg" alt="Tennext Soft office" class="reveal">
  </div>
</section>
<section style="background:linear-gradient(180deg,#eef1fb,#f7f8fd)">
  <div class="container center">
...
<section>
  <div class="container">
    ...
    <div class="cta-band reveal" style="margin-top:2.6rem">
      ...
      <p>Visit us in Tambaram or start with a WhatsApp message.</p>
...
file_put_contents("$dir/about.html", shell(
    'About Us — Tennext Soft',
    'Tennext Soft is a Chennai software company in Tambaram building affordable websites, apps and business software.',
...
<section>
  <div class="container">
    <div class="contact-cards">
      <div class="ccard reveal"><div class="ci">✆</div><b>Call Us</b><a href="tel:+919840422538">{$PHONE}</a></div>
      <div class="ccard reveal"><div class="ci">✦</div><b>WhatsApp</b><a href="https://wa.me/919840422538?text=Hi%20Tennext%20Soft!" target="_blank" rel="noopener">Chat with us now</a></div>
    ...
    <div style="margin-top:3rem">
      ...
      <div style="max-width:760px;margin-top:1.5rem">
        ...
        <details class="reveal"><summary>Can we meet in person?</summary><p>Of course! We're in Tambaram, Chennai. Call or WhatsApp to fix a time — or we can visit your shop or office for a demo.</p></details>
      </div>
    </div>
    <div class="cta-band reveal" style="margin-top:2.8rem">
      <h2>Ready when you are</h2>
      <p>{$PHONE} · {$EMAIL}</p>
      <a href="https://wa.me/919840422538?text=Hi%20Tennext%20Soft!%20I%20want%20a%20quote." class="btn btn-white" target="_blank" rel="noopener">WhatsApp Us Now</a>
    </div>
  </div>
</section>
HTML;
file_put_contents("$dir/contact.html", shell(
    'Contact — Tennext Soft',
    'Contact Tennext Soft, Tambaram, Chennai. Call +91 98404 22538 or email contact@tennextsoft.com.',
    $CSS, nav('contact'), $body, $footer, $JS));

echo "Tennext Soft site generated: index, services, products, about, contact + " . count($images) . " images\n";
