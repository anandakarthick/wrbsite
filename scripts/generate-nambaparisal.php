<?php
// Generate nambaparisal.com website: 5-page static site, emerald/teal theme
$base = dirname(__DIR__);
$dir = $base . '/public/sites/nambaparisal';
@mkdir($dir, 0777, true);
...
$NAME = 'nambaparisal IT Software Developments';
$SHORT = 'nambaparisal';
$PHONE = '+91 70945 14970';
$PHONE_RAW = '917094514970';
$EMAIL = 'sales@nambaparisal.com';
$ADDRESS = 'Ashok Nagar, Chennai - 600083, Tamil Nadu, India';

$CSS = <<<CSS
:root{--g1:#059669;--g2:#0d9488;--g3:#84cc16;--ink:#0a1f17;--body:#2d4a3e;--muted:#557466;--soft:#6b8779;--bg:#f5fbf8;--grad:linear-gradient(135deg,#059669 0%,#0d9488 55%,#84cc16 100%)}
...
.btn-primary{background:var(--grad);color:#fff;box-shadow:0 8px 20px rgba(5,150,105,.3)}
.btn-primary:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(13,148,136,.4)}
.btn-outline{background:#fff;color:var(--g1);border:2px solid rgba(5,150,105,.35)}
.btn-outline:hover{background:rgba(5,150,105,.07)}
...
.totop{position:fixed;bottom:1.6rem;left:1.6rem;width:46px;height:46px;border-radius:50%;background:var(--grad);color:#fff;border:none;cursor:pointer;font-size:1rem;box-shadow:0 10px 24px rgba(5,150,105,.35);opacity:0;visibility:hidden;transform:translateY(10px);transition:.3s;z-index:150}
...
header{position:fixed;top:0;left:0;right:0;z-index:100;background:rgba(255,255,255,.85);backdrop-filter:blur(14px);border-bottom:1px solid rgba(10,31,23,.06);transition:.3s}
header.scrolled{background:rgba(255,255,255,.97);box-shadow:0 10px 24px rgba(10,31,23,.08)}
...
.badge{display:inline-block;padding:.45rem 1.1rem;background:rgba(5,150,105,.09);border:1px solid rgba(5,150,105,.3);border-radius:.5rem;font-size:.8rem;font-weight:700;color:var(--g1);margin-bottom:1rem;letter-spacing:.04em;text-transform:uppercase}
...
.hero{padding:9.5rem 0 0;background:linear-gradient(160deg,#eafaf2 0%,#f5fbf8 55%,#f2fae8 100%);position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;width:520px;height:520px;border-radius:50%;background:radial-gradient(circle,rgba(132,204,22,.16),transparent 65%);top:-180px;right:-140px}
.hero::after{content:'';position:absolute;width:420px;height:420px;border-radius:50%;background:radial-gradient(circle,rgba(13,148,136,.14),transparent 65%);bottom:-80px;left:-120px}
...
.hero-img img{display:block;width:100%;border-radius:1.4rem;box-shadow:0 30px 60px rgba(10,31,23,.2);border:6px solid #fff}
...
.page-hero{padding:8.5rem 0 3.5rem;background:linear-gradient(135deg,#065f46,#0f766e);color:#fff;position:relative;overflow:hidden}
.page-hero::after{content:'';position:absolute;width:380px;height:380px;border-radius:50%;background:rgba(132,204,22,.2);filter:blur(70px);top:-120px;right:-80px}
...
.card{background:#fff;border:1px solid rgba(10,31,23,.07);border-left:4px solid var(--g1);border-radius:1rem;padding:1.7rem;transition:.35s}
.card:hover{transform:translateY(-6px);box-shadow:0 18px 40px rgba(5,150,105,.16);border-left-color:var(--g3)}
...
.img-card{background:#fff;border:1px solid rgba(10,31,23,.07);border-radius:1rem;overflow:hidden;transition:.35s}
.img-card:hover{transform:translateY(-6px);box-shadow:0 18px 40px rgba(5,150,105,.16)}
...
.checklist li{display:flex;gap:.7rem;background:#fff;border:1px solid rgba(10,31,23,.07);border-radius:.8rem;padding:.95rem 1.15rem;font-size:.93rem;transition:.3s}
.checklist li:hover{transform:translateY(-3px);border-color:rgba(5,150,105,.4)}
...
.step::before{content:'';position:absolute;left:21px;top:46px;bottom:0;width:2px;background:rgba(5,150,105,.25)}
...
.plan{background:#fff;border:1px solid rgba(10,31,23,.08);border-radius:1rem;padding:1.9rem 1.6rem;display:flex;flex-direction:column;transition:.35s;position:relative}
.plan:hover{transform:translateY(-7px);box-shadow:0 18px 40px rgba(5,150,105,.16)}
...
.quote{margin-top:2.4rem;padding:2rem;background:linear-gradient(135deg,rgba(5,150,105,.06),rgba(132,204,22,.07));border:1px solid rgba(5,150,105,.2);border-radius:1rem}
.quote blockquote{font-style:italic;color:#173b2d;font-size:1.02rem;margin-bottom:1rem}
.quote b{color:var(--ink);font-size:.9rem}.quote span{color:var(--soft);font-size:.8rem;display:block}
details{background:#fff;border:1px solid rgba(10,31,23,.07);border-radius:.8rem;margin-bottom:.7rem;overflow:hidden}
details[open]{border-color:rgba(5,150,105,.4)}
summary{padding:1rem 1.35rem;cursor:pointer;font-weight:600;color:#12332а;color:#12332a;list-style:none;display:flex;justify-content:space-between;gap:1rem}
...
.ccard{background:#fff;border:1px solid rgba(10,31,23,.07);border-radius:1rem;padding:1.6rem;text-align:center;transition:.3s}
.ccard:hover{transform:translateY(-5px);border-color:rgba(5,150,105,.4)}
...
.split img{border-radius:1.2rem;box-shadow:0 22px 45px rgba(10,31,23,.15)}
footer{background:#0a2019;color:#9fc4b3;padding:3rem 0 1.5rem}
...
footer a{color:#9fc4b3;font-size:.9rem}
footer a:hover{color:#84cc16}
.fbrand{display:flex;align-items:center;gap:.6rem;font-weight:800;font-size:1.05rem;color:#fff;margin-bottom:.8rem}
.fbottom{border-top:1px solid rgba(255,255,255,.12);padding-top:1.3rem;text-align:center;font-size:.83rem;color:#7ba692}
.fbottom a{color:#84cc16;font-weight:600}
...
@media(max-width:900px){
 ...
 .nav ul{position:fixed;top:64px;right:-100%;width:78%;max-width:300px;height:100vh;background:rgba(255,255,255,.98);flex-direction:column;align-items:flex-start;padding:2rem;gap:1.2rem;transition:.3s;box-shadow:-10px 0 30px rgba(10,31,23,.12)}
 .nav ul.open{right:0}
...
<header>
  <nav class="nav">
    <a href="index.html" class="brand"><img src="logo.png" alt="nambaparisal logo" class="brand-logo"><span>nambaparisal<small>IT Software Developments</small></span></a>
    <ul>{$links}</ul>
...
<footer>
  <div class="container">
    <div class="fgrid">
      <div>
        <div class="fbrand"><img src="logo.png" alt="nambaparisal logo" class="brand-logo"> nambaparisal IT Software Developments</div>
      ...
      <div>
        <h4>Contact</h4>
        <ul>
          <li><a href="tel:+917094514970">📞 {$PHONE}</a></li>
          <li><a href="mailto:{$EMAIL}">✉ {$EMAIL}</a></li>
          <li>📍 {$ADDRESS}</li>
        </ul>
      </div>
    </div>
    <div class="fbottom">© <span class="yr"></span> nambaparisal IT Software Developments · All rights reserved · <a href="https://nambaparisal.com/">nambaparisal.com</a></div>
  </div>
</footer>
<a class="wa-float" href="https://wa.me/917094514970?text=Hi%20nambaparisal!%20I%20need%20a%20software%20solution." target="_blank" rel="noopener" aria-label="WhatsApp">
...
<section class="hero">
  <div class="container hero-grid">
    ...
    <div class="hero-img reveal"><img src="hero.jpg" alt="Technology solutions by nambaparisal"></div>
  </div>
</section>
...
<section style="background:linear-gradient(180deg,#eafaf2,#f5fbf8)">
  <div class="container split">
    <img src="about.jpg" alt="Our team at work" class="reveal">
    <div>
      <span class="badge">Why nambaparisal</span>
...
<section>
  <div class="container">
    <div class="quote reveal">
      ...
      <b>Murugan R</b><span>Retail Shop Owner, Ashok Nagar</span>
    </div>
...
file_put_contents("$dir/index.html", shell(
    'nambaparisal IT Software Developments — Websites, Apps & Business Software in Chennai',
    'Website development, mobile apps, e-commerce, billing and custom software in Ashok Nagar, Chennai. Call +91 70945 14970.',
...
<section style="background:linear-gradient(180deg,#eafaf2,#f5fbf8)">
  <div class="container">
    ...
    <div class="steps">
      <div class="step reveal"><div class="step-n">1</div><div><h3>Free Discussion</h3><p>Tell us your need — over call, WhatsApp or chai at our Ashok Nagar office. We understand before we quote.</p></div></div>
...
file_put_contents("$dir/services.html", shell(
    'Services — nambaparisal IT Software Developments',
...
file_put_contents("$dir/products.html", shell(
    'Products — nambaparisal IT Software Developments',
...
<section>
  <div class="container split">
    <div>
      ...
      <p style="color:var(--muted);margin-bottom:1rem">nambaparisal IT Software Developments is a software company based in Ashok Nagar, Chennai. We started with a simple observation: small and medium businesses around us were paying too much for software they didn't understand, or struggling with tools that didn't fit how they actually work.</p>
    ...
    <img src="office.jpg" alt="nambaparisal office" class="reveal">
  </div>
</section>
<section style="background:linear-gradient(180deg,#eafaf2,#f5fbf8)">
  <div class="container center">
...
<section>
  <div class="container">
    ...
    <div class="cta-band reveal" style="margin-top:2.6rem">
      ...
      <p>Visit us in Ashok Nagar or start with a WhatsApp message.</p>
...
file_put_contents("$dir/about.html", shell(
    'About Us — nambaparisal IT Software Developments',
    'nambaparisal IT Software Developments is a Chennai software company in Ashok Nagar building affordable websites, apps and business software.',
...
<section>
  <div class="container">
    <div class="contact-cards">
      <div class="ccard reveal"><div class="ci">✆</div><b>Call Us</b><a href="tel:+917094514970">{$PHONE}</a></div>
      <div class="ccard reveal"><div class="ci">✦</div><b>WhatsApp</b><a href="https://wa.me/917094514970?text=Hi%20nambaparisal!" target="_blank" rel="noopener">Chat with us now</a></div>
    ...
    <div style="margin-top:3rem">
      ...
      <div style="max-width:760px;margin-top:1.5rem">
        ...
        <details class="reveal"><summary>Can we meet in person?</summary><p>Of course! We're in Ashok Nagar, Chennai. Call or WhatsApp to fix a time — or we can visit your shop or office for a demo.</p></details>
      </div>
    </div>
    <div class="cta-band reveal" style="margin-top:2.8rem">
      <h2>Ready when you are</h2>
      <p>{$PHONE} · {$EMAIL}</p>
      <a href="https://wa.me/917094514970?text=Hi%20nambaparisal!%20I%20want%20a%20quote." class="btn btn-white" target="_blank" rel="noopener">WhatsApp Us Now</a>
    </div>
  </div>
</section>
HTML;
file_put_contents("$dir/contact.html", shell(
    'Contact — nambaparisal IT Software Developments',
    'Contact nambaparisal IT Software Developments, Ashok Nagar, Chennai. Call +91 70945 14970 or email sales@nambaparisal.com.',
    $CSS, nav('contact'), $body, $footer, $JS));

echo "nambaparisal site generated: index, services, products, about, contact + " . count($images) . " images\n";
