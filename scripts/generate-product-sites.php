...
.totop{position:fixed;bottom:1.6rem;right:1.6rem;width:46px;height:46px;border-radius:50%;background:var(--pgrad);color:#fff;border:none;cursor:pointer;font-size:1rem;box-shadow:0 10px 24px rgba(37,99,235,.35);opacity:0;visibility:hidden;transform:translateY(10px);transition:.3s;z-index:150}
...
#sbot{position:fixed;bottom:1.6rem;left:1.6rem;z-index:180}
...
.sbot-panel{position:absolute;bottom:66px;left:0;width:340px;max-width:calc(100vw - 2.5rem);height:440px;max-height:calc(100vh - 130px);display:flex;flex-direction:column;background:rgba(255,255,255,.97);backdrop-filter:blur(14px);border:1px solid rgba(37,99,235,.18);border-radius:1.2rem;box-shadow:0 24px 60px rgba(11,18,32,.25);overflow:hidden;opacity:0;visibility:hidden;transform:translateY(14px) scale(.97);transition:.3s cubic-bezier(.22,1,.36,1)}
...
#swa{position:fixed;bottom:6.3rem;left:1.6rem;z-index:180}
...
(function(){
 ...
 root.innerHTML='<button type="button" class="sbot-toggle" aria-label="Chat with us">💬<span class="sbot-pulse"></span></button>'
 +'<div class="sbot-panel"><div class="sbot-head"><div class="av">🤖</div><div><b>'+NAME+' Assistant</b><span>● Online — ask me anything</span></div></div>'
